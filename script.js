// License verification for MSM Portfolio
(function() {
    // Configuration
    const projectName = "MSM Portfolio";
    const projectPath = "/dev";
    const allowedDomains = ["msmelok.github.io"];
    ...
    const isDomainAllowed = allowedDomains.some(domain => {
        // Handle localhost variations (with or without ports)
        if (domain === 'localhost') {
            return currentDomain === 'localhost' || currentDomain.startsWith('localhost:');
        }
        
        // Handle 127.0.0.1 variations (with or without ports)
        if (domain === '127.0.0.1') {
            return currentDomain === '127.0.0.1' || currentDomain.startsWith('127.0.0.1:');
        }
    ...
    // Check if current path starts with project path
    const isPathAllowed = currentPath.startsWith(projectPath);
    
    // Verify license
    if (!isDomainAllowed || !isPathAllowed) {
        const deniedUrl = './denied.html?domain=' + encodeURIComponent(currentDomain) + '&project=' + encodeURIComponent(projectName);
        window.location.href = deniedUrl;
        return;
    }
    
    // License verification successful
    console.log('✓ License verified for ' + projectName + ' on ' + currentDomain + currentPath);
})();
